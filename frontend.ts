// src/frontend.ts — Column Layout Extension
...
(function (ctx: any) {
  ...
  function buildColumnCSS(cols: number): string {
    return `
  ...
  function applyColumns(cols: number): void {
    if (removeStyle) removeStyle();
    removeStyle = ctx.dom.addStyle(buildColumnCSS(cols));
  }

  // ─── Drawer tab UI ────────────────────────────────────────────────────────
  function buildPreviewGrid(cols: number): void {
    if (!previewCells.length) return;
    const grid = previewCells[0].parentElement as HTMLElement | null;
    if (!grid) return;
    grid.style.gridTemplateColumns = `repeat(${cols}, minmax(0, 1fr))`;
  }

  function updateUI(cols: number): void {
    if (sliderEl) sliderEl.value = String(cols);
    if (labelEl) labelEl.textContent = cols === 1 ? "1 column" : `${cols} columns`;
    buildPreviewGrid(cols);
  }

  function renderDrawerTab(root: HTMLElement, initialColumns: number): void {
    root.innerHTML = `
      <div style="
        padding: 18px 16px;
        display: flex;
    ...
    sliderEl = root.querySelector<HTMLInputElement>("#cl-col-slider");
    labelEl = root.querySelector<HTMLElement>("#cl-col-label");
    previewCells = Array.from(root.querySelectorAll<HTMLElement>(".cl-preview-cell"));
  ...
  // Re-apply on DOM mutations (handles route/view changes in the SPA)
